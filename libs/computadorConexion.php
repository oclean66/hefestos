...
$cn=  conectar();
       ...
       $sql = 'SELECT idcomputador, idtransaccion, fechaprestamo, computador.idconexion, IMEI, numero,
       usuario.nombre as nombre, agencia.nombre as nombreag, grupo.nombre as nombregr, 
       vendedor.nombre as nombreven, banca.nombre as nombreban,  modelo, banca.idbanca,
        vendedor.idvendedor, grupo.idgrupo,agencia.idagencia, conexionnombre
...
?>
 ...
 <script type="text/javascript" language="javascript" src="js/jslistado.js"></script>
            ...
            <table cellpadding="0" cellspacing="0" border="0" class="display" id="tabla_lista_computadorCN">
                <thead>
                    <tr>
                  ...
                  <tbody>
                   ...
                   while($reg=  mysql_fetch_array($listado))
                   {
                              ...
                              echo '<td >'.mb_convert_encoding($reg['IMEI'], "iso-8859-1").'</td>';   
                              echo '<td >'.mb_convert_encoding($reg['numero'], "iso-8859-1").'</td>';   
                              echo '<td >'.mb_convert_encoding($reg['conexionnombre'], "iso-8859-1").'</td>';
                              echo '<td >'.mb_convert_encoding($reg['modelo'], "iso-8859-1").'</td>';
                             
                              echo '<td ><a href="#">'.mb_convert_encoding($reg['idagencia'].' '.$reg['nombreag'], "iso-8859-1").'</a></td>';
                              echo '<td ><a href="#">'.mb_convert_encoding($reg['idgrupo'].' '.$reg['nombregr'], "iso-8859-1").'</a></td>';
                                 echo '
                                <td>
