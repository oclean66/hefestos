...
class CrugeUserManager {
    ...
    /*
     * Core Cruge modificado para usarse en siam system, carga los usuarios de cierto grupo
     */

    public function loadUsersByGCCD($gccd_id) {
        $user = CrugeFactory::get()->getICrugeStoredUserList(array('GCCD_Id' => $gccd_id));
//        if (($boolAndLoadFields == true) && ($user != null)) {
//            $this->loadUserFields($user);
//        }
        return $user;
    ...
    public function getLabelField(ICrugeField $field) {
        ...
        $r = "class='control-label col-sm-2'";
        $ast = "";
        ...
        switch ($field->fieldtype) {
            ...
            case CRUGEFIELDTYPE_LISTBOX:
                return CHtml::dropDownList(
                                $name, $field->getFieldValue(), $arOpt, $htmlOpt
                        ) . "\n";
        }
        return null;
    }
     ...
     */
    public function createRandomUsers($size, $role_name = 'default', $firstname_field = 'firstname', $lastname_field = 'lastname') {
    ...
    $fieldmap, $_values, $modality, &$errorResult, $debug = false, $role_name = 'default', // aplican cuando modality es 'auto'
