<?php 
	...
	if((count($roles) > 0) && ($model->type != CAuthItem::TYPE_TASK)){
		...
		foreach($roles as $item){
			...
			$treeDataRoles[] = array(
				'id'=>$item->name,
				'text'=>"<span class='{$asignado} {$loop}'>".CrugeTranslator::t("classes",$item->name)
					."</span>".$imgPin,
			...
			foreach($taskinfo['topmenu'] as $topTask){
				...
				$treeDataMenu[] = array(
					...
					'text'=>"<span class='itemtext authitemtop {$asignado} {
						$loop}'>".CrugeTranslator::t("classes",$text)
						."</span>".$imgPin,
					'expanded'=>false,
			...
			foreach($taskinfo['orphan'] as $orpTask){
				...
				$treeDataError[] = array(
					...
					'text'=>"<span class='{$asignado} {$loop}'>".
						CrugeTranslator::t("classes",$rbac->getTaskText($orpTask))."</span>".$imgPin,
					'expanded'=>false,
					'hasChildren'=>false,
					'htmlOptions'=>array('class'=>'authitem'
						, 'alt'=>$orpTask->name),
		...
		foreach($listacatg as $catg_filter => $catg_name){
			...
			foreach($rbac->getOperationsFiltered($catg_filter, $operaciones) 
				...
				$childs[] = array(
					'id'=>$item->name,
					'text'=>"<span class='{$asignado} {$loop}'>".
						CrugeTranslator::t("classes",$item->name)."</span>".$imgPin,
					'htmlOptions'=>array('class'=>'authitem',
						'alt'=>$item->name),
				);
			}
			...
			$treeDataOps[] = array(
				'text'=>CrugeTranslator::t("classes",$catg_name),
