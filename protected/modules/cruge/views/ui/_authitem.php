...
<div class='row' <?php echo $colorEspecialBkTaskTipoMenuitem; ?> >
    <div class='col authname'><?php echo CrugeTranslator::t('classes',$data->name); ?></div>


    <div class='col operacion'>
        <?php echo CHtml::link(CrugeTranslator::t("propiedades"), Yii::app()->user->ui->getRbacAuthItemUpdateUrl($data->name));
        ?>
    </div>

    <?php if ($data->type != CAuthItem::TYPE_OPERATION) { ?>
        <div class='col operacion'>
            <?php echo CHtml::link(CrugeTranslator::t("editar permisos"), Yii::app()->user->ui->getRbacAuthItemChildItemsUrl($data->name));
            ?>
        </div>
    <?php } ?>
