...
<div class="box box-bordered">
    <div class="box-title">
        <h3>
            <i class="fa fa-th-list"></i><?php echo ucfirst(CrugeTranslator::t("Opciones avanzadas")); ?></h3>
    </div>
    <div class="box-content nopadding">
        <form action="#" method="POST" class="form-vertical form-bordered form-striped">
            <div class="form-group">
                <?php echo $form->labelEx($model, 'state'); ?>
                <?php echo $form->dropDownList($model, 'state', Yii::app()->user->um->getUserStateOptions(), array('class' => 'form-control')); ?>
                <?php echo $form->error($model, 'state'); ?>
            </div>
            <div class="form-group">
                <label for="password" class="control-label">Reactivacion</label>
                ...
                <?php if ($model->state == CRUGEUSERSTATE_NOTACTIVATED) { ?>
                    ...
                    <?php echo CHtml::ajaxbutton(ucfirst(CrugeTranslator::t("reenviar correo de actissvacion")), Yii::app()->user->ui->getAjaxResendRegistrationEmailUrl($model->getPrimaryKey()), array('success' => 'js:fnSuccess'), array('class' => 'btn btn-primary')); ?>
                    <p class='hint' id='resendStatus'>
                        <?php echo ucfirst(CrugeTranslator::t("Esta accion creara una nueva clave.")); ?>
                    </p>

                <?php } else { ?>
                    <input type='button' class='btn btn-primary' value='<?php echo ucfirst(CrugeTranslator::t("Reenviar correo de activacion")) ?>' disabled='disabled'>
                <?php } ?>

            </div>

            <div class="form-group">
                <label for="textarea" class="control-label"><?php echo ucfirst(CrugeTranslator::t("asignacion de roles")); ?></label>
                <p class='hind'>
