...
class TccaController extends Controller
{
	...
	public function actionView($id)
	{
		...
		foreach ($tableros as $tablero) {
			$boards = Tcca::model()->find('TCCA_Type=0 and TCCA_Id =:id order by TCCA_Archived, TCCA_Name', array(':id' => $tablero->TCCM_IdModel));
			if (isset($boards))
				$dataProvider[] = array(
					'TCCA_Id' => $boards->TCCA_Id,
					'TCCA_Name' => $boards->TCCA_Name,
					'TCCA_Archived' => $boards->TCCA_Archived
				);
			# code...
		}
